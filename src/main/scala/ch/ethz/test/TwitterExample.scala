...
object TwitterExample {
  def main(args: Array[String]) {
    ...
    val streamSource = if(params.has(TwitterSource.CONSUMER_KEY) &&
                          params.has(TwitterSource.CONSUMER_SECRET) &&
                          params.has(TwitterSource.TOKEN) &&
                          params.has(TwitterSource.TOKEN_SECRET))
                            env.addSource(new TwitterSource(params.getProperties))
                       else
                            env.fromElements(TwitterExampleData.TEXTS)

      val tweets = streamSource.flatMap(x => {
          val jsonParser = new ObjectMapper()
          val jsonNode = jsonParser.readValue(x.toString, classOf[JsonNode])
          val isEnglish = jsonNode.has("user") && jsonNode.get("user").has("lang") && jsonNode.get("user").get("lang").asText().equals("en")
          val hasText = jsonNode.has("text")

          if(isEnglish && hasText){
            jsonNode.get("text").asText().toLowerCase().split("\\s+").map(x=>(x,1))
          }
          else
            List()
      })
        .keyBy(0).sum(1).filter(_._2==100)
        .print

    // execute program
    env.execute("Flink Streaming Twitter example")
  }
}


object TwitterExampleData {
  val TEXTS: List[String] = List(
  ...
  val STREAMING_COUNTS_AS_TUPLES: String = "(apache,1)\n" + "(apache,2)\n" + "(apache,3)\n" + "(flink,1)\n" + "(flink,2)\n" + "(flink,3)\n"
